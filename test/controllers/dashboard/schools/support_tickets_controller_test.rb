  ...
  module Schools
    class InternshipApplicationsTest < ActionDispatch::IntegrationTest
      ...
      def submit_fails_and_renders_new(params)
        ...
        post dashboard_support_tickets_path(@school, params: params)
      ...
      test 'As SchoolManager POST support ticket redirects to new if parameter week_ids is not compliant' do
        @params[:support_tickets_school_manager][:week_ids] = []
        submit_fails_and_renders_new(@params)
      end

      test 'As SchoolManager POST support ticket redirects to new if students_quantity parameter is not compliant' do
        @params[:support_tickets_school_manager][:students_quantity] = 'string'
        submit_fails_and_renders_new(@params)
      end

      test 'As SchoolManager POST support ticket redirects to new if class_rooms_quantity parameters is not compliant' do
