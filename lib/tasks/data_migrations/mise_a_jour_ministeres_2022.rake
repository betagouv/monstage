namespace :data_migrations do
  ...
  task mise_a_jour_ministeres_2022: :environment do
    ...
    ministeres_a_rattacher.each do |ministere_a_rattacher|
      ancien_ministere = Group.find_by(name: ministere_a_rattacher[0])
      nouveau_ministere = Group.find_by(name: ministere_a_rattacher[1])
    
      if ancien_ministere.present?
    ...
    Group.find_or_create_by(name: "Ministère de la Transition énergétique", is_public: true, is_paqte: false)
    Group.find_or_create_by(name: "Ministère de la Transformation et de la Fonction publiques", is_public: true, is_paqte: false)
  end
end
