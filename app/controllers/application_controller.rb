class ApplicationController < ActionController::Base
  ...
  def find_selectable_weeks
    ...
    if current_month <= InternshipOffer.MONTH_OF_YEAR_SHIFT # Before the end of May, offers should be available from now until May of the current year
      @current_weeks = Week.from_date_to_date_for_year(today, Date.new(current_year, InternshipOffer.MONTH_OF_YEAR_SHIFT, InternshipOffer.DAY_OF_YEAR_SHIFT), current_year)
      ...
      @current_weeks = Week.from_date_until_end_of_year(first_day_available, current_year)
                           .or(Week.from_date_to_date_for_year(Date.new(current_year + 1), Date.new(current_year + 1, InternshipOffer.MONTH_OF_YEAR_SHIFT, InternshipOffer.DAY_OF_YEAR_SHIFT), current_year + 1))
    end
  end
end
