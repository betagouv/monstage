...
export default function SirenInput({
  ...
  const displayItemResult = (item) => {
    const siege = item.etablissementSiege ? ' [siège]' : '';
    const denomination = `${item.uniteLegale.sigleUniteLegale || item.uniteLegale.denominationUniteLegale} - `;
    const city = item.adresseEtablissement.libelleCommuneEtablissement.toLowerCase();
    const address = `${item.adresseEtablissement.numeroVoieEtablissement} ${item.adresseEtablissement.typeVoieEtablissement ? item.adresseEtablissement.typeVoieEtablissement.toLowerCase() : ''} ${item.adresseEtablissement.libelleVoieEtablissement.toLowerCase()} - ${city.charAt(0).toUpperCase() + city.slice(1)}`;
    return denomination + address + siege;
  };
                            ...
                            <li
                              {...getItemProps({
                                ...
                                style: {
                                  ...
                                  color: item.etablissementSiege ? 'red' : ''
                                },
                              })}
                            >
