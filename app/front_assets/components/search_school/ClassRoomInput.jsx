        ...
        {hasPendingSuggestion && (
          <>
            ...
            >
              ...
              {Object.keys(classRoomsSuggestionsByType).length > 1 && Object.keys(classRoomsSuggestionsByType).map(schoolTrack => (
                <optgroup label={TRADS[schoolTrack]}>
                  {classRoomsSuggestionsByType[schoolTrack].map(renderClassRoomOption)}
                </optgroup>
              ))}

              {Object.keys(classRoomsSuggestionsByType).length <= 1 && (classRoomsSuggestions || []).map(renderClassRoomOption)}
            </select>
            <label htmlFor={`${resourceName}_class_room_id`}>Classe</label>
          </>
        )}
